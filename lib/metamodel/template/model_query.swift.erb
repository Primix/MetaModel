public extension <%= model.name %> {
    ...
    <% model.properties_exclude_id.each do |property| %><%= """static func findBy(#{property.name} #{property.name}: #{property.type_without_optional}) -\> #{model.relation_name} {
        return #{model.relation_name}().findBy(#{property.name}: #{property.name})
    }""" %>
    <% end %>
